...
- Level k start index: getLevelStart6(k) = 2^k + k - 1
- Level k length: getLevelLength6(k) = 2^k + 1  
...
size_t getLeftChildSiblingIndex6(size_t index, size_t depth) {
    size_t currentLevel = findLevel6(index, depth);
...
size_t getRightChildSiblingIndex6(size_t index, size_t depth) {
    size_t currentLevel = findLevel6(index, depth);
...
void SegmentTree6::init(MPCTIO &tio, yield_t & yield) {
    ...
    // Initialize leftChildSiblingArray with the help of getLeftChildSiblingIndex6 function
    auto leftChildSiblingArray = leftChildSibling.flat(tio, yield);
    leftChildSiblingArray.init([this] (size_t i) -> size_t {
        if (i < num_items) {
            return getLeftChildSiblingIndex6(i, depth);
        } else {
            return size_t(0);
        }
    });

    // Initialize rightChildSiblingArray with the help of getRightChildSiblingIndex6 function
    auto rightChildSiblingArray = rightChildSibling.flat(tio, yield);
    rightChildSiblingArray.init([this] (size_t i) -> size_t {
        if (i < num_items) {
            return getRightChildSiblingIndex6(i, depth); // if even (right) keep sibling else 0
        } else {
            return size_t(0);
        }
    });

    // Initialize parentArray with the help of parentIndex6 function
...
void SegmentTree6::getBitVector(MPCTIO &tio, MPCIO &mpcio, yield_t & yield, Duoram < RegXS > &bitVec, RegAS leftLevelIndex, RegAS rightLevelIndex) {
    ...
    for(uint32_t i=1; i<=depth; i++) {
        ...
        // --- if parent of left and right are same then the marking of bitvector is already completed so we set isDone here ---        
        ...
        // --- we check wether leftLevelIndex and rightLevelIndex are valid or not i.e. checking wether leftLevelIndex <= rightLevelIndex ---
        ...
        // --- we have to mark the leftSibling and rightSibling only if they are valid and marking is not yet completed ---
        RegBS Check;
        mpc_and(tio, yield, Check, one ^ isDone, valid); // one ^ isDone => !isDone

        // --- based on Check set marking for leftSibling and rightSibling ---
        ...
        // --- Update leftLevelIndex and rightLevelIndex for next iteration ---
