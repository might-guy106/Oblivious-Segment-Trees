...
class SegmentTreeTest {
    ...
    public:
        size_t num_items;
        size_t depth;
        SegmentTreeTest(int player_num, size_t size, size_t d) : oram(player_num, size), isEven(player_num, size), sibling(player_num, size), parent(player_num, size) {
            num_items = size;
            depth = d;
    ...
    // Detailed Update with per-operation timing (for comparison)
    void Update_Detailed(MPCTIO &tio, MPCIO &mpcio, yield_t & yield, RegAS index, RegAS value);
    // Test function to isolate Flat object creation overhead
    void Test_FlatCreation(MPCTIO &tio, MPCIO &mpcio, yield_t & yield);
    // Test 1: Fresh local ORAM with depth number of RegAS accesses
    void Test_LocalORAM(MPCTIO &tio, MPCIO &mpcio, yield_t & yield);
    // Test 2: Use existing SegTreeArray depth number of times
    void Test_SegTreeArray(MPCTIO &tio, MPCIO &mpcio, yield_t & yield);
    // Test 3: Level-wise Flat objects with accesses
    void Test_LevelWiseAccess(MPCTIO &tio, MPCIO &mpcio, yield_t & yield);
