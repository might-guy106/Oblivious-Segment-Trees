...
- Sibling indices are obtained via a separate Sibling ORAM (SiblingOram) that
  ...
  index. This is the key distinction of SegmentTree9 versus variants that
...
// bitVec)
RegAS SegmentTree9::computeRangeSum(MPCTIO &tio, MPCIO &mpcio, yield_t &yield, RegXS leftLevelIndex,
...
void SegTree9(MPCIO &mpcio, const PRACOptions &opts, char **args) {
    ...
    exp_id_stream << "st9_d" << (int)depth << "_u" << n_updates << "_q" << n_queries << "_"
