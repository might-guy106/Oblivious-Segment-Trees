...
class SegmentTree4 {
    ...
    public:
        size_t num_items;
        size_t depth;
        SegmentTree4(int player_num, size_t size, size_t d) : oram(player_num, size), isEven(player_num, size), nextL(player_num, size), nextR(player_num, size), parent(player_num, size) {
            num_items = size;
            depth = d;
