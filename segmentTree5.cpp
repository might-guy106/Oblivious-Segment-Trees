...
void SegmentTree5::init(MPCTIO &tio, yield_t & yield) {
   ...
   isEvenArray.init([this] (size_t i) -> size_t {
    ...
    siblingArray.init([this] (size_t i) -> size_t {
        if (i >= 1 && i < num_items) {
            return arrayIndexToLevelPos5((i % 2 == 0) ? i + 1 : i - 1); // we took 1s sibling as 0 and 0s as 1 which should not cause any issue.
        } else {
            return size_t(0);
        }
    });
    ...
    parentArray.init([this] (size_t i) -> size_t {
        if (i >= 1 && i < num_items) {
            return arrayIndexToLevelPos5(i/2);
        } else {
            return size_t(0);
        }
    });
...
void SegmentTree5::getBitVector(MPCTIO &tio, MPCIO &mpcio, yield_t & yield, Duoram < RegXS > &bitVec, RegAS left, RegAS right) {
