...
void SegmentTree10::init(MPCTIO &tio, yield_t &yield) {
  ...
  SiblingArray.init([this](size_t i) -> size_t {
    if (i >= 1 && i < num_items) {
      return arrayIndexToLevelIndex(
          (i % 2 == 0) ? (i + 1)
                       : (i - 1)); // even index store right sibling, odd index
                                   // store left sibling (level index)
    } else {
      return size_t(0);
    }
  });
...
void SegmentTree10::printSegmentTree(MPCTIO &tio, yield_t &yield) {
  auto SegTreeArray = TreeOram.flat(tio, yield);
  auto SegTreeRecons = SegTreeArray.reconstruct();
  for (size_t i = 1; i < num_items; i++) {
    std::cout << "SegTreeArray[" << i << "] = " << SegTreeRecons[i].share()
              << std::endl;
  }
}
...
// bitVec)
RegAS SegmentTree10::computeRangeSum(MPCTIO &tio, MPCIO &mpcio, yield_t &yield,
                                     RegXS leftLevelIndex,
                                     RegXS rightLevelIndex,
  ...
  auto step1_duration = std::chrono::duration_cast<std::chrono::milliseconds>(
                            step1_end_time - start_time)
                            .count();
  std::cout << "Step 1 (Path Computation) Time: " << step1_duration << " ms"
            << std::endl;
  ...
  for (uint32_t i = 1; i <= depth; i++) {
    ...
    sum_coroutines.emplace_back([&tio, level, i, this, &leftPathIndex,
                                 &rightPathIndex, &SegTreeArray, &SiblingArray,
      ...
      typename Duoram<RegAS>::Flat levelTreeArray(SegTreeArray, tio, sub_yield,
                                                  levelStart, levelLength);
      typename Duoram<RegAS>::Flat levelSiblingArray(
          SiblingArray, tio, sub_yield, levelStart, levelLength);
      ...
      RegAS diff1 =
          rightIndexAS - (leftIndexAS + one); // diff1 = right - (left + 1)
      auto [lt_c1, eq_c1, gt_c1] =
          cdpf1.compare(tio, sub_yield, diff1, tio.aes_ops());
      ...
      run_coroutines(
          sub_yield,
          [&tio, &levelTreeArray, &levelSiblingArray, isNotDone, leftIndex,
           &leftSum, leftLastBit](yield_t &yield) {
            ...
            mpc_and(tio, yield, isLeftIncluded, isNotDone,
                    leftLastBit ^ tio.player());
            mpc_flagmult(tio, yield, leftSum, isLeftIncluded, leftSibValue);
          },
          [&tio, &levelTreeArray, &levelSiblingArray, isNotDone, rightIndex,
           &rightSum, rightLastBit](yield_t &yield) {
            auto levelSiblingArrayCoro = levelSiblingArray.context(yield);
            auto levelTreeArrayCoro = levelTreeArray.context(yield);
      ...
      if (i == 1) {
        ...
        auto [lt_c2, eq_c2, gt_c2] =
            cdpf2.compare(tio, sub_yield, diff2, tio.aes_ops());

        RegAS leftLeafContribution;
        RegAS rightLeafContribution;
        run_coroutines(
            sub_yield,
            [&tio, &lt_c2, &eq_c2, &gt_c2, &zero, &one, &leftIndex,
             &levelTreeArray, &leftLeafContribution](yield_t &yield) {
              // if right >= left add left
              // that is equivalent to !(left > right) => lt_c2 ^ tio.player()
              auto levelTreeArrayCoro = levelTreeArray.context(yield);
              RegAS leftLeafValue = levelTreeArrayCoro[leftIndex];
              mpc_select(tio, yield, leftLeafContribution, lt_c2 ^ tio.player(),
                         zero, leftLeafValue);
            },
            [&tio, &lt_c2, &eq_c2, &gt_c2, &zero, &one, &rightIndex,
             &levelTreeArray, &rightLeafContribution](yield_t &yield) {
              ...
              mpc_select(tio, yield, rightLeafContribution, gt_c2, zero,
                         rightLeafValue);
            });
  ...
  auto step2_duration = std::chrono::duration_cast<std::chrono::milliseconds>(
                            step2_end_time - step1_end_time)
                            .count();

  std::cout << "Step 2 (Direct Sum Computation) Time: " << step2_duration
            << " ms" << std::endl;

  // Log metrics if logger is provided
  if (logger) {
    logger->log_metric("query", operation_id, "path_computation_time",
                       step1_duration);
    logger->log_metric("query", operation_id, "direct_sum_time",
                       step2_duration);
  }
...
void SegmentTree10::RangeSum(MPCTIO &tio, MPCIO &mpcio, yield_t &yield,
                             RegXS left, RegXS right, PerformanceLogger *logger,
                             size_t operation_id) {
  RegAS sum =
      computeRangeSum(tio, mpcio, yield, left, right, logger, operation_id);
...
void SegmentTree10::Update(MPCTIO &tio, MPCIO &mpcio, yield_t &yield,
                           RegXS index, RegAS value, PerformanceLogger *logger,
                           size_t operation_id) {
  ...
  typename Duoram<RegAS>::Flat leafLevelArray(SegTreeArray, tio, yield,
...
// Debugging and intermediate reconstructions
  ...
  std::cout << "Index to be updated in the leaf level array = " << recons_index
            << std::endl;
  std::cout << "Index to be updated in the segment tree array = "
            << leafStart + recons_index << std::endl;
  ...
  auto phase1_duration = std::chrono::duration_cast<std::chrono::milliseconds>(
                             phase1_end - update_start)
                             .count();
  std::cout << "Update Phase 1 (Path Computation) Time: " << phase1_duration
            << " ms" << std::endl;
  ...
  for (size_t i = 1; i <= depth; i++) {
    ...
    update_coroutines.emplace_back([&tio, level, this, &diff, &updatePathIndex,
...
#ifdef SEGTREE_VERBOSE
      auto recons_Index =
          mpc_reconstruct(tio, sub_yield, updatePathIndex[level]);
      auto recons_updated = mpc_reconstruct(
          tio, sub_yield, levelTreeArray[updatePathIndex[level]]);
      std::cout << "Updated Index = " << (levelStart + recons_Index)
                << " with value = " << recons_updated << std::endl;
#endif
    });
  }
  ...
  auto phase2_duration = std::chrono::duration_cast<std::chrono::milliseconds>(
                             phase2_end - phase1_end)
                             .count();

  std::cout << "Update Phase 2 (Parallel Updates) Time: " << phase2_duration
            << " ms" << std::endl;

  // Log metrics if logger is provided
  if (logger) {
    logger->log_metric("update", operation_id, "path_computation_time",
                       phase1_duration);
    logger->log_metric("update", operation_id, "parallel_updates_time",
                       phase2_duration);
  }
}
...
void SegTree10(MPCIO &mpcio, const PRACOptions &opts, char **args) {
  ...
  exp_id_stream << "st9_d" << (int)depth << "_u" << n_updates << "_q"
                << n_queries << "_" << std::put_time(&tm, "%Y%m%d_%H%M%S");
  std::string experiment_id = exp_id_stream.str();

  // Initialize logger (only player 0 will actually create log files)
  PerformanceLogger logger(experiment_id, depth, n_updates, n_queries,
                           tio.player(), tio.getMode());

  run_coroutines(tio, [&tio, &mpcio, len, depth, n_updates, n_queries,
                       &logger](yield_t &yield) {
    // Time initialization
    auto init_start = std::chrono::high_resolution_clock::now();

    SegmentTree10 segTree(tio.player(), len, depth);
    ...
    oss << "Initialization Time: " << init_duration.count() / 1000.0
        << " seconds\n";
...
// Print initial segment tree
    ...
    for (size_t u = 0; u < n_updates; ++u) {
      ...
      auto single_update_duration =
          std::chrono::duration_cast<std::chrono::milliseconds>(
              single_update_end - single_update_start)
              .count();

      logger.log_metric("update", u + 1, "total_update_time",
                        single_update_duration);
    ...
    oss.str("");
    oss << "Total Updates Time: " << update_duration.count() / 1000.0
        << " seconds\n";
    logger.log_output(oss.str());

    logger.log_metric("update_summary", 0, "total_all_updates_time",
...
// Print updated segment tree
    ...
    for (size_t q = 0; q < n_queries; ++q) {
      logger.log_section("Range Sum Query " + std::to_string(q + 1) +
                         " begins");
      ...
      segTree.RangeSum(tio, mpcio, yield, left_index, right_index, &logger,
                       q + 1);

      auto single_query_end = std::chrono::high_resolution_clock::now();
      auto single_query_duration =
          std::chrono::duration_cast<std::chrono::milliseconds>(
              single_query_end - single_query_start)
              .count();

      logger.log_metric("query", q + 1, "total_query_time",
    ...
    oss.str("");
    oss << "Total Range Queries Time: " << query_duration.count() / 1000.0
        << " seconds\n";
    logger.log_output(oss.str());

    logger.log_metric("query_summary", 0, "total_all_queries_time",
