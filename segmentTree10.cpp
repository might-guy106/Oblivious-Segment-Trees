...
size_t arrayIndexToLevelIndex10(size_t idx) {
...
void SegmentTree10::init(MPCTIO &tio, yield_t &yield) {
    ...
    SiblingArray.init([this](size_t i) -> size_t {
        if (i >= 1 && i < num_items) {
            return arrayIndexToLevelIndex10((i % 2 == 0) ? (i + 1) : (i - 1)); // even index store right sibling, odd
...
// bitVec)
RegAS SegmentTree10::computeRangeSum(MPCTIO &tio, MPCIO &mpcio, yield_t &yield, RegXS leftLevelIndex,
...
void SegmentTree10::RangeSum(MPCTIO &tio, MPCIO &mpcio, yield_t &yield, RegXS left, RegXS right,
...
void SegmentTree10::Update(MPCTIO &tio, MPCIO &mpcio, yield_t &yield, RegXS index, RegAS value,
                           PerformanceLogger *logger, size_t operation_id) {
...
void SegTree10(MPCIO &mpcio, const PRACOptions &opts, char **args) {
    ...
    exp_id_stream << "st10_d" << (int)depth << "_u" << n_updates << "_q" << n_queries << "_"
