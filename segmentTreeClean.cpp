...
size_t arrayIndexToLevelPosClean(size_t idx) {
...
void SegmentTreeClean::init(MPCTIO &tio, yield_t & yield) {
   ...
   isEvenArray.init([this] (size_t i) -> size_t {
    ...
    siblingArray.init([this] (size_t i) -> size_t {
        if (i >= 1 && i < num_items) {
            return arrayIndexToLevelPosClean((i % 2 == 0) ? i + 1 : i - 1); // we took 1s sibling as 0 and 0s as 1 which should not cause any issue.
        } else {
            return size_t(0);
        }
    });
    ...
    parentArray.init([this] (size_t i) -> size_t {
        if (i >= 1 && i < num_items) {
            return arrayIndexToLevelPosClean(i/2);
        } else {
            return size_t(0);
        }
    });
...
void SegmentTreeClean::printSegmentTree(MPCTIO &tio, yield_t & yield) {
    auto SegTreeArray = oram.flat(tio, yield);
...
void SegmentTreeClean::getBitVector(MPCTIO &tio, MPCIO &mpcio, yield_t & yield, Duoram < RegXS > &bitVec, RegAS left, RegAS right) {
...
void SegmentTreeClean::RangeSum(MPCTIO &tio,  MPCIO &mpcio, yield_t & yield, RegAS left, RegAS right) {
...
void SegmentTreeClean::Update(MPCTIO &tio, MPCIO &mpcio, yield_t & yield, RegAS index, RegAS value) {
    auto SegTreeArray = oram.flat(tio, yield);
    auto parentArray = parent.flat(tio, yield);
...
void SegTreeClean(MPCIO &mpcio, const PRACOptions &opts, char **args) {
    ...
    run_coroutines(tio, [&tio, &mpcio, len, depth, n_updates, n_queries] (yield_t &yield) {
        ...
        SegmentTreeClean segTree(tio.player(), len, depth);
