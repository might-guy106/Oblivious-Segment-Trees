...
#include "options.hpp"
#include "preproc.hpp"
#include "preprocessinglogger.hpp"
...
static void comp_player_main(boost::asio::io_context &io_context,
  ...
  io_context.run();
  t.join();
  std::cout << "Player " << player << " completed\n";
...
static void server_player_main(boost::asio::io_context &io_context,
  ...
  boost::asio::post(io_context, [&] {
    if (opts.mode == MODE_PREPROCESSING) {
      std::unique_ptr<PreprocessingLogger> logger =
          std::make_unique<PreprocessingLogger>();
      ...
      std::chrono::duration<double> distinct_time = end_time - start_time;

      logger->log_duration("execution_time", distinct_time.count());
    } else {
...
int main(int argc, char **argv) {
  ...
  if (player >
      2) { // player 0 storing nothing, player 1 storing player 0's address,
           // player 2 storing player 0's and player 1's address
    usage(argv[0]);
  }
  if (player > 0) {
    if (*args == NULL) {
