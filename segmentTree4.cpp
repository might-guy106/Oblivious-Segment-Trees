...
size_t arrayIndexToLevelPos4(size_t idx) {
...
void SegmentTree4::init(MPCTIO &tio, yield_t & yield) {
    ...
    nextLArray.init([this] (size_t i) -> size_t {
        if (i >= 1 && i < num_items) {
            if (i > 1 && ((i + 1) & i) == 0) // i is one less than a power of two.
                return ((arrayIndexToLevelPos4(i) + 1)/2 - 1);
            else
                return ((arrayIndexToLevelPos4(i) + 1)/2);
        } else {
            return size_t(0);
        }
    });
...
void SegmentTree4::printSegmentTree(MPCTIO &tio, yield_t & yield) {
    auto SegTreeArray = oram.flat(tio, yield);
...
void SegmentTree4::getBitVector(MPCTIO &tio, yield_t & yield, Duoram < RegXS > &bitVec, RegAS left, RegAS right) {
...
void SegmentTree4::RangeSum(MPCTIO &tio, yield_t & yield, RegAS left, RegAS right) {
...
void SegmentTree4::Update(MPCTIO &tio, yield_t & yield, RegAS index, RegAS value) {
    auto SegTreeArray = oram.flat(tio, yield);
    auto parentArray = parent.flat(tio, yield);
...
void SegTree4(MPCIO &mpcio, const PRACOptions &opts, char **args) {
    ...
    run_coroutines(tio, [&tio, len, depth, n_updates, n_queries, &mpcio] (yield_t &yield) {
        SegmentTree4 segTree(tio.player(), len, depth);
        segTree.init(tio, yield);

        std::cout << "===== Segment Tree 4 Init Stats =====" << std::endl;
        ...
        std::cout << "\n===== Range Sum Queries Stats =====" << std::endl;
    });
}