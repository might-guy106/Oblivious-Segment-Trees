...
static void online_test(MPCIO &mpcio, const PRACOptions &opts, char **args) {
  ...
  coroutines.emplace_back([&tio, &A, nbits](yield_t &yield) {
    mpc_mul(tio, yield, A[2], A[0], A[1], nbits);
  });
  coroutines.emplace_back([&tio, &A, V, nbits](yield_t &yield) {
    mpc_valuemul(tio, yield, A[3], V, nbits);
  });
  coroutines.emplace_back([&tio, &A, &F0, nbits](yield_t &yield) {
    mpc_flagmult(tio, yield, A[5], F0, A[4], nbits);
  });
  coroutines.emplace_back([&tio, &A, &F1, nbits](yield_t &yield) {
    mpc_oswap(tio, yield, A[6], A[7], F1, nbits);
  });
  coroutines.emplace_back([&tio, &A, &X, nbits](yield_t &yield) {
    mpc_xs_to_as(tio, yield, A[8], X, nbits);
  });
  coroutines.emplace_back([&tio, &AX, &F0, nbits](yield_t &yield) {
    mpc_select(tio, yield, AX[2], F0, AX[0], AX[1], nbits);
  });
  coroutines.emplace_back([&tio, &FA, &F0, &F1](yield_t &yield) {
    mpc_and(tio, yield, FA, F0, F1);
  });
  coroutines.emplace_back([&tio, &FO, &F0, &F1](yield_t &yield) {
    mpc_or(tio, yield, FO, F0, F1);
  });
  coroutines.emplace_back([&tio, &FS, &F0, &F1, &F2](yield_t &yield) {
      ...
      if (tio.player() == 2) {
        ...
        for (int i = 0; i < 3; ++i) {
          ...
          for (nbits_t level = min_level; level <= depth; ++level) {
            ...
            if (tio.player() == 1) {
              tio.iostream_peer() << dpf.li[depth - level].scaled_xor
            ...
            for (address_t x = 0; x < (address_t(1) << level); ++x) {
              ...
              if (tio.player() == 1) {
                ...
                if (is_nonzero) {
                  ...
                  printf("\nSCALE                  ");
                  for (nbits_t j = 0; j < WIDTH; ++j) {
                    printf(" %016lx %016lx", peer_scaled_xor[j].xshare,
...
static value_t parallel_streameval_rdpf(MPCTIO &tio, const RDPF<1> &dpf,
  ...
  for (int thread_num = 0; thread_num < num_threads; ++thread_num) {
    ...
    boost::asio::post(pool, [&tio, &dpf, &scaled_xor, &aes_ops, thread_num,
                             threadstart, threadsize] {
...
static void rdpfeval_timing(MPCIO &mpcio, const PRACOptions &opts,
                            char **args) {
  nbits_t depth = 6;
  address_t start = 0;
...
static void par_rdpfeval_timing(MPCIO &mpcio, const PRACOptions &opts,
                                char **args) {
  nbits_t depth = 6;
  address_t start = 0;
  ...
  run_coroutines(tio, [&tio, depth, start, num_threads](yield_t &yield) {
    if (tio.player() == 2) {
      ...
      for (int i = 0; i < 2; ++i) {
        ...
        auto pe = ParallelEval(dpf, start, 0, address_t(1) << depth,
                               num_threads, tio.aes_ops());
        RDPF<1>::RegXSW result, init;
        result = pe.reduce(init, [&dpf](int thread_num, address_t i,
                                        const RDPF<1>::LeafNode &leaf) {
...
static void tupleeval_timing(MPCIO &mpcio, const PRACOptions &opts,
                             char **args) {
  nbits_t depth = 6;
  address_t start = 0;
...
static void par_tupleeval_timing(MPCIO &mpcio, const PRACOptions &opts,
                                 char **args) {
  nbits_t depth = 6;
  address_t start = 0;
  ...
  run_coroutines(tio, [&tio, depth, start, num_threads](yield_t &yield) {
    ...
    if (tio.player() == 2) {
      RDPFPair<1> dp = tio.rdpfpair(yield, depth);
      auto pe = ParallelEval(dp, start, 0, address_t(1) << depth, num_threads,
                             aes_ops);
      ...
      RDPFTriple<1> dt = tio.rdpftriple(yield, depth);
      auto pe = ParallelEval(dt, start, 0, address_t(1) << depth, num_threads,
                             aes_ops);
...
// T is RegAS or RegXS for additive or XOR shared database respectively
template <typename T>
static void duoram(MPCIO &mpcio, const PRACOptions &opts, char **args) {
  nbits_t depth = 6;
  int items = 4;
...
// T is RegAS or RegXS for additive or XOR shared database respectively
template <typename T>
static void read_test(MPCIO &mpcio, const PRACOptions &opts, char **args) {
  nbits_t depth = 6;
  int items = 4;
      ...
      run_coroutines(tio, [&tio, query, target, iters](yield_t &yield) {
        ...
        for (int i = 0; i < iters; ++i) {
          if (tio.player() == 2) {
            ...
            printf("ashare = %016lX\nxshare = %016lX\n", dpf.as_target.ashare,
                   dpf.xs_target.xshare);
...
static int compare_test_one(MPCTIO &tio, yield_t &yield, value_t target,
                            value_t x) {
  ...
  if (player == 2) {
    ...
    printf("%016lx %016lx %d %d %d %d ", target, x, lt.bshare, eq.bshare,
           gt.bshare, eeq.bshare);
    // Check the answer
    if (player == 1) {
...
static int compare_test_target(MPCTIO &tio, yield_t &yield, value_t target,
                               value_t x) {
  int res = 1;
...
static void bsearch_test(MPCIO &mpcio, const PRACOptions &opts, char **args) {
  value_t target;
  arc4random_buf(&target, sizeof(target));
  target >>= 1;
  nbits_t depth = 6;
  ...
  MPCTIO tio(mpcio, 0, opts.num_cpu_threads);
  run_coroutines(tio, [&tio, &mpcio, depth, len, iters, target,
                       is_presorted](yield_t &yield) {
    RegAS tshare;
    std::cout << "\n===== SETUP =====\n";
...
template <typename T>
static void related(MPCIO &mpcio, const PRACOptions &opts, char **args) {
  nbits_t depth = 5;
  ...
  run_coroutines(tio, [&mpcio, &tio, depth, layer](yield_t &yield) {
    ...
    // bit length of idx).
    typename Duoram<T>::template OblivIndex<RegXS, 3> oidx(tio, yield, idx,
                                                           layer);
...
template <typename T>
static void path(MPCIO &mpcio, const PRACOptions &opts, char **args) {
  nbits_t depth = 5;
