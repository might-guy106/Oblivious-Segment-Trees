...
static void online_test(MPCIO &mpcio, const PRACOptions &opts, char **args) {
    ...
    if (!is_server) {
        ...
        printf("A:\n");
        for (size_t i = 0; i < as_memsize; ++i)
            printf("%3lu: %016lX\n", i, A[i].ashare);
        printf("AX:\n");
        for (size_t i = 0; i < xs_memsize; ++i)
            printf("%3lu: %016lX\n", i, AX[i].xshare);
        printf("V  : %016lX\n", V);
        printf("F0 : %01X\n", F0.bshare);
        printf("F1 : %01X\n", F1.bshare);
    ...
    coroutines.emplace_back([&tio, &A, nbits](yield_t &yield) { mpc_mul(tio, yield, A[2], A[0], A[1], nbits); });
    coroutines.emplace_back([&tio, &A, V, nbits](yield_t &yield) { mpc_valuemul(tio, yield, A[3], V, nbits); });
    coroutines.emplace_back(
        [&tio, &A, &F0, nbits](yield_t &yield) { mpc_flagmult(tio, yield, A[5], F0, A[4], nbits); });
    coroutines.emplace_back([&tio, &A, &F1, nbits](yield_t &yield) { mpc_oswap(tio, yield, A[6], A[7], F1, nbits); });
    coroutines.emplace_back([&tio, &A, &X, nbits](yield_t &yield) { mpc_xs_to_as(tio, yield, A[8], X, nbits); });
    coroutines.emplace_back(
        [&tio, &AX, &F0, nbits](yield_t &yield) { mpc_select(tio, yield, AX[2], F0, AX[0], AX[1], nbits); });
    coroutines.emplace_back([&tio, &FA, &F0, &F1](yield_t &yield) { mpc_and(tio, yield, FA, F0, F1); });
    coroutines.emplace_back([&tio, &FO, &F0, &F1](yield_t &yield) { mpc_or(tio, yield, FO, F0, F1); });
    coroutines.emplace_back([&tio, &FS, &F0, &F1, &F2](yield_t &yield) { mpc_select(tio, yield, FS, F2, F0, F1); });
    ...
    if (mpcio.player == 1) {
        ...
        for (size_t i = 0; i < as_memsize; ++i)
            S[i] = A[i].ashare + B[i].ashare;
        for (size_t i = 0; i < xs_memsize; ++i)
            Y[i] = AX[i].xshare ^ BAX[i].xshare;
...
template <nbits_t WIDTH> static void rdpf_test(MPCIO &mpcio, const PRACOptions &opts, char **args, bool incremental) {
    nbits_t depth = 6;
    size_t num_iters = 1;
    ...
    run_coroutines(tio, [&tio, depth, num_iters, incremental](yield_t &yield) {
        ...
        for (size_t iter = 0; iter < num_iters; ++iter) {
            if (tio.player() == 2) {
                RDPFPair<WIDTH> dp = tio.rdpfpair<WIDTH>(yield, depth, incremental);
                for (int i = 0; i < 2; ++i) {
                ...
                RDPFTriple<WIDTH> dt = tio.rdpftriple<WIDTH>(yield, depth, incremental);
                for (int i = 0; i < 3; ++i) {
                    ...
                    for (nbits_t level = min_level; level <= depth; ++level) {
                        ...
                        if (tio.player() == 1) {
                            tio.iostream_peer() << dpf.li[depth - level].scaled_xor << dpf.li[depth - level].scaled_sum;
                        } else {
                        ...
                        for (address_t x = 0; x < (address_t(1) << level); ++x) {
                            ...
                            if (tio.player() == 1) {
                                ...
                                tio.iostream_peer() >> peer_ub >> peer_ua >> peer_sx >> peer_sa;
                                ub ^= peer_ub;
                                ua += peer_ua;
                                sx ^= peer_sx;
                                sa += peer_sa;
                                ...
                                if (is_nonzero) {
                                    ...
                                    printf("\nSCALE                  ");
                                    for (nbits_t j = 0; j < WIDTH; ++j) {
                                        printf(" %016lx %016lx", peer_scaled_xor[j].xshare, peer_scaled_sum[j].ashare);
                                    }
                                    printf("\n");
                                }
                            }
            ...
            run_coroutines(tio, [&tio, depth](yield_t &yield) {
                ...
                if (tio.player() == 2) {
                    ...
                    for (int i = 0; i < 2; ++i) {
                        ...
                        printf("%016lx\n%016lx\n", scaled_xor[0].xshare, dpf.li[0].scaled_xor[0].xshare);
                        printf("\n");
                    }
                } else {
...
static value_t parallel_streameval_rdpf(MPCTIO &tio, const RDPF<1> &dpf, address_t start, int num_threads) {
    ...
    for (int thread_num = 0; thread_num < num_threads; ++thread_num) {
        ...
        boost::asio::post(pool, [&tio, &dpf, &scaled_xor, &aes_ops, thread_num, threadstart, threadsize] {
            // printf("Thread %d from %X for %X\n", thread_num, threadstart,
            // threadsize);
...
static void rdpfeval_timing(MPCIO &mpcio, const PRACOptions &opts, char **args) {
    ...
    run_coroutines(tio, [&tio, depth, start, num_threads](yield_t &yield) {
        if (tio.player() == 2) {
            ...
            for (int i = 0; i < 2; ++i) {
                RDPF<1> &dpf = dp.dpf[i];
                value_t scaled_xor = parallel_streameval_rdpf(tio, dpf, start, num_threads);
                printf("%016lx\n%016lx\n", scaled_xor, dpf.li[0].scaled_xor[0].xshare);
                printf("\n");
            }
        } else {
            RDPFTriple<1> dt = tio.rdpftriple(yield, depth);
...
static void par_rdpfeval_timing(MPCIO &mpcio, const PRACOptions &opts, char **args) {
    nbits_t depth = 6;
    address_t start = 0;
    ...
    run_coroutines(tio, [&tio, depth, start, num_threads](yield_t &yield) {
        if (tio.player() == 2) {
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                auto pe = ParallelEval(dpf, start, 0, address_t(1) << depth, num_threads, tio.aes_ops());
                RDPF<1>::RegXSW result, init;
                result = pe.reduce(init, [&dpf](int thread_num, address_t i, const RDPF<1>::LeafNode &leaf) {
                    return dpf.scaled_xs(leaf);
                });
                printf("%016lx\n%016lx\n", result[0].xshare, dpf.li[0].scaled_xor[0].xshare);
                printf("\n");
            }
        } else {
            RDPFTriple<1> dt = tio.rdpftriple(yield, depth);
...
static void tupleeval_timing(MPCIO &mpcio, const PRACOptions &opts, char **args) {
    ...
    run_coroutines(tio, [&tio, depth, start](yield_t &yield) {
        ...
        if (tio.player() == 2) {
            ...
            printf("%016lx\n%016lx\n", scaled_xor0[0].xshare, dp.dpf[0].li[0].scaled_xor[0].xshare);
            printf("\n");
            printf("%016lx\n%016lx\n", scaled_xor1[0].xshare, dp.dpf[1].li[0].scaled_xor[0].xshare);
            printf("\n");
        } else {
            RDPFTriple<1> dt = tio.rdpftriple(yield, depth);
            ...
            printf("%016lx\n%016lx\n", scaled_xor0[0].xshare, dt.dpf[0].li[0].scaled_xor[0].xshare);
            printf("\n");
            printf("%016lx\n%016lx\n", scaled_xor1[0].xshare, dt.dpf[1].li[0].scaled_xor[0].xshare);
            printf("\n");
            printf("%016lx\n%016lx\n", scaled_xor2[0].xshare, dt.dpf[2].li[0].scaled_xor[0].xshare);
            printf("\n");
        }
    });
}

static void par_tupleeval_timing(MPCIO &mpcio, const PRACOptions &opts, char **args) {
    nbits_t depth = 6;
    address_t start = 0;
    ...
    run_coroutines(tio, [&tio, depth, start, num_threads](yield_t &yield) {
        ...
        if (tio.player() == 2) {
            RDPFPair<1> dp = tio.rdpfpair(yield, depth);
            auto pe = ParallelEval(dp, start, 0, address_t(1) << depth, num_threads, aes_ops);
            RDPFPair<1>::RegXSWP result, init;
            result = pe.reduce(init, [&dp](int thread_num, address_t i, const RDPFPair<1>::LeafNode &leaf) {
                RDPFPair<1>::RegXSWP scaled;
                dp.scaled(scaled, leaf);
                return scaled;
            });
            printf("%016lx\n%016lx\n", std::get<0>(result)[0].xshare, dp.dpf[0].li[0].scaled_xor[0].xshare);
            printf("\n");
            printf("%016lx\n%016lx\n", std::get<1>(result)[0].xshare, dp.dpf[1].li[0].scaled_xor[0].xshare);
            printf("\n");
        } else {
            RDPFTriple<1> dt = tio.rdpftriple(yield, depth);
            auto pe = ParallelEval(dt, start, 0, address_t(1) << depth, num_threads, aes_ops);
            RDPFTriple<1>::RegXSWT result, init;
            result = pe.reduce(init, [&dt](int thread_num, address_t i, const RDPFTriple<1>::LeafNode &leaf) {
                RDPFTriple<1>::RegXSWT scaled;
                dt.scaled(scaled, leaf);
                return scaled;
            });
            printf("%016lx\n%016lx\n", std::get<0>(result)[0].xshare, dt.dpf[0].li[0].scaled_xor[0].xshare);
            printf("\n");
            printf("%016lx\n%016lx\n", std::get<1>(result)[0].xshare, dt.dpf[1].li[0].scaled_xor[0].xshare);
            printf("\n");
            printf("%016lx\n%016lx\n", std::get<2>(result)[0].xshare, dt.dpf[2].li[0].scaled_xor[0].xshare);
            printf("\n");
        }
    });
}
...
template <typename T> static void duoram_test(MPCIO &mpcio, const PRACOptions &opts, char **args) {
    nbits_t depth = 6;
    ...
    run_coroutines(tio, [&tio, depth, share, len](yield_t &yield) {
        ...
        std::vector<T> Av = A[std::array{aidx, aidx2, aidx3}];
        ...
        A[std::array{aidx, aidx2, aidx3}] -= std::array{Aw1, Aw2, Aw3};
...
// T is RegAS or RegXS for additive or XOR shared database respectively
template <typename T> static void duoram(MPCIO &mpcio, const PRACOptions &opts, char **args) {
    nbits_t depth = 6;
    int items = 4;
    ...
    run_coroutines(tio, [&mpcio, &tio, depth, items](yield_t &yield) {
        ...
        address_t mask = (depth < ADDRESS_MAX_BITS ? ((address_t(1) << depth) - 1) : ~0);
...
// T is RegAS or RegXS for additive or XOR shared database respectively
template <typename T> static void read_test(MPCIO &mpcio, const PRACOptions &opts, char **args) {
    nbits_t depth = 6;
    int items = 4;
...
static void cdpf_test(MPCIO &mpcio, const PRACOptions &opts, char **args) {
...
static int compare_test_one(MPCTIO &tio, yield_t &yield, value_t target, value_t x) {
    ...
    if (player == 2) {
        ...
        printf("%016lx %016lx %d %d %d %d ", target, x, lt.bshare, eq.bshare, gt.bshare, eeq.bshare);
        // Check the answer
        if (player == 1) {
            ...
            tio.iostream_peer() >> peer_xsh >> peer_lt >> peer_eq >> peer_gt >> peer_eeq;
            lt ^= peer_lt;
            eq ^= peer_eq;
            gt ^= peer_gt;
            eeq ^= peer_eeq;
...
static int compare_test_target(MPCTIO &tio, yield_t &yield, value_t target, value_t x) {
    int res = 1;
...
static void compare_test(MPCIO &mpcio, const PRACOptions &opts, char **args) {
    value_t target, x;
    arc4random_buf(&target, sizeof(target));
...
static void sort_test(MPCIO &mpcio, const PRACOptions &opts, char **args) {
    nbits_t depth = 6;
    ...
    run_coroutines(tio, [&tio, depth, len](yield_t &yield) {
        ...
        std::vector<coro_t> coroutines;
        for (address_t i = 0; i < size; ++i) {
            coroutines.emplace_back([&A, i](yield_t &yield) {
        ...
        if (tio.player() == 0) {
            for (address_t i = 0; i < size; ++i) {
                ...
                if (i > 0 && i < len && check[i].share() < check[i - 1].share()) {
                    fail = true;
                }
            }
            if (fail) {
...
static void pad_test(MPCIO &mpcio, const PRACOptions &opts, char **args) {
    nbits_t depth = 6;
...
template <typename T, bool basic> static void bsearch_test(MPCIO &mpcio, const PRACOptions &opts, char **args) {
    value_t target;
    arc4random_buf(&target, sizeof(target));
    target >>= 1;
    nbits_t depth = 6;
    ...
    run_coroutines(tio, [&tio, &mpcio, depth, len, iters, target, is_presorted](yield_t &yield) {
        ...
        if (is_presorted) {
            A.init([](size_t i) { return value_t(i) << 16; });
        } else {
            A.explicitonly(true);
            for (address_t i = 0; i < len; ++i) {
                RegAS v;
        ...
        if (tio.player() == 0) {
            for (address_t i = 0; i < len; ++i) {
                ...
                if (i == checkindex) {
                    ...
                    // should be < target
                    if ((i < len && check[i].share() < checktarget) || (i > 0 && check[i - 1].share() >= checktarget)) {
                        fail = true;
                    }
                }
            }
...
template <typename T> static void related(MPCIO &mpcio, const PRACOptions &opts, char **args) {
    nbits_t depth = 5;
...
template <typename T> static void path(MPCIO &mpcio, const PRACOptions &opts, char **args) {
    nbits_t depth = 5;
